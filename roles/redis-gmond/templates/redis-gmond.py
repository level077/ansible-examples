        ...
        try:
          for line in info.splitlines()[1:]:
              ...
              if "#" == line[0]:
                  continue
              n, v = line.split(":")
	      n = metric_handler.metric_prefix + n
              if n in metric_handler.descriptors:
                  if n == metric_handler.metric_prefix + "master_sync_status":
                      v = 1 if v == 'up' else 0
                  if n == metric_handler.metric_prefix + "db0":
                      v = v.split('=')[1].split(',')[0]
                  if n == metric_handler.metric_prefix + "used_memory":
                      v = int(int(v) / 1000)
                  if n == metric_handler.metric_prefix + "total_connections_received":
                      ...
                      if metric_handler.prev_total_connections == 0:
                          ...
                          metric_handler.prev_total_connections = int(v)
                          v = cps
                  if n == metric_handler.metric_prefix + "total_commands_processed":
...
def metric_init(params={}):
    ...
    metric_handler.metric_prefix = params['host'] + "_" + params['port'] + "_"
    metrics = {
        metric_handler.metric_prefix + "connected_clients": {"units": "clients"},
        metric_handler.metric_prefix + "connected_slaves": {"units": "slaves"},
        metric_handler.metric_prefix + "blocked_clients": {"units": "clients"},
        metric_handler.metric_prefix + "used_memory": {"units": "KB"},
        metric_handler.metric_prefix + "rdb_changes_since_last_save": {"units": "changes"},
        metric_handler.metric_prefix + "rdb_bgsave_in_progress": {"units": "yes/no"},
        metric_handler.metric_prefix + "master_sync_in_progress": {"units": "yes/no"},
        metric_handler.metric_prefix + "master_link_status": {"units": "yes/no"},
        #"aof_bgrewriteaof_in_progress": {"units": "yes/no"},
        metric_handler.metric_prefix + "total_connections_received": { "units": "connections/sec" },
        metric_handler.metric_prefix + "instantaneous_ops_per_sec": {"units": "ops"},
        metric_handler.metric_prefix + "total_commands_processed": { "units": "commands/sec" },
        metric_handler.metric_prefix + "expired_keys": {"units": "keys"},
        metric_handler.metric_prefix + "pubsub_channels": {"units": "channels"},
        metric_handler.metric_prefix + "pubsub_patterns": {"units": "patterns"},
        #"vm_enabled": {"units": "yes/no"},
        metric_handler.metric_prefix + "master_last_io_seconds_ago": {"units": "seconds ago"},
        metric_handler.metric_prefix + "db0": {"units": "keys"},
    }
    ...
    for name, updates in metrics.iteritems():
        descriptor = {
            ...
            #"groups": "redis",
	    "groups": "redis_" + params['host'] + "_" + params['port'],
        }
